...
impl<W: Write> StreamRecorder<W> {
    ...
    pub fn write_frame(
        ...
        let de_len_bytes = (detector_events_rle.len() as u16).to_le_bytes();
        self.encoder.write_all(&de_len_bytes)?;
        hasher.update(&de_len_bytes);
        self.encoder.write_all(detector_events_rle)?;
        hasher.update(detector_events_rle);
        ...
        if !parity_checks.is_empty() {
            ...
            hasher.update(parity_checks);
        }

        // Frame terminator: 0xFFFF sentinel + CRC32 of the full frame (header + payload).
        self.encoder.write_all(&0xFFFFu16.to_le_bytes())?;
        self.encoder.write_all(&hasher.finalize().to_le_bytes())?;
