...
Quantum error correction is still a young field, and the tools that will make fault-tolerant computing real should be built in the open. stabstream is released under Apache 2.0 so that hardware teams, decoder researchers, and quantum software developers can inspect every layer of the stack, contribute improvements, and build on a shared foundation — rather than duplicating infrastructure or working around proprietary black boxes. We believe open-source real-time QEC tooling accelerates the entire field.
